  ...
  const load = useCallback(()=>{
    ...
    fetch(endpoint)
      ...
      .then(json=>{
        ...
        console.log('Leaderboard normalized list length:', Array.isArray(list) ? list.length : 'n/a');
        setData(list);
      }).catch(err=>{
  ...
  return (
    <div className="card">
      <div className="card-body">
        <h3 className="card-title h4">Leaderboard</h3>
        ...
        {visible.length === 0 ? (
          <div className="text-muted">No leaderboard entries to display.</div>
        ) : (
