...
export default function Workouts(){
  ...
  const endpoint = `${BASE_API_URL}/workouts/`;
  ...
  const load = useCallback(()=>{
    ...
    fetch(endpoint)
      ...
      .then(json=>{
        ...
        console.log('Workouts normalized list length:', Array.isArray(list) ? list.length : 'n/a');
        setData(list);
      }).catch(err=>{
  ...
  return (
    <div className="card">
      <div className="card-body">
        <h3 className="card-title h4">Workouts</h3>
        ...
        {visible.length === 0 ? (
          <div className="text-muted">No workouts to display.</div>
        ) : (
